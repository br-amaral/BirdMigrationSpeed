...
bird <- read.csv("arrival_master_2020-07-21.csv")
bird <- subset(bird, VALID_GAM == TRUE)
greenup <- read.csv("MidGreenup_2020-08-06-forest.csv")
...
cells <- unique(greenup[,c("cell","cell_lat","cell_lng")])  ### DATA PTs  


adj = dnearneigh(as.matrix(cells[,3:2]),0,360, longlat = TRUE)  ### a list of adjacent cells for each cell
...
for (a in 2002:2017){
  for (spp in 1:length(spec)){
    dat.temp <- subset(bird,year == a & species == spec[spp])
    for (b in 1:nrow(cells)){
      ...
      if(nrow(dat.lm) >= minNeigh) {  ### validate that minimum data points available
        if(sum(!is.na(dat.lm$arr_GAM_mean))>=minNeigh){
          ...
          velocity <- rbind(velocity, c(spec[spp],a,cells[b,1],sum(!is.na(dat.lm$arr_GAM_mean)),1/sqrt(sum(coef^2)),angle))
        }
      }
    }
  }
...
velocity = data.frame(species = velocity[,1],year = as.numeric(velocity[,2]),
                      cell = velocity[,3],NArr = as.numeric(velocity[,4]),
                      vArrMag = as.numeric(velocity[,5]),vArrAng = as.numeric(velocity[,6]))
...
velocityG = data.frame(year = as.numeric(velocityG[,1]),
                      cell = velocityG[,2],NGr = as.numeric(velocityG[,3]),
                      vGrMag = as.numeric(velocityG[,4]),vGrAng = as.numeric(velocityG[,5]))
