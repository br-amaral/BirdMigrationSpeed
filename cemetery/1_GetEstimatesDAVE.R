...
# import data ---------------------------
BIRDDATA_PATH <- glue("arrival_master_2020-07-21.csv") # may not be the most current data file - but what i first found
GREENDATA_PATH <- glue("MidGreenup_2020-08-06-forest.csv")
...
cells <- cells %>% 
  mutate(adj = adj,
         numn = num)
# create weights for the regression analysis
bird <- bird %>% 
  mutate(weight = exp(-(year - 2008)/9))
...
for (yr in 2002:2017){
  for (spp in 1:length(spec)){
    ...
    for (b in 1:nrow(cells)){ # look at the neighbours of a cell   c(adj[[b]], cells[b,1])
      ...
      dat.lm <- dat.temp[dat.temp$cell %in% vecnei, c("cell_lat","cell_lng","arr_GAM_mean","cell","weight")]  
      rm(vecnei)
      ...
      if(nrow(dat.lm) >= minNeigh) {  # validate that minimum data points available
        if(sum(!is.na(dat.lm$arr_GAM_mean)) >= minNeigh + 1){
          ...
          colnames(xy) <- c('x','y')
          coef <- lm(data = dat.lm, formula = arr_GAM_mean ~ xy$x + xy$y, weights=weight)$coef[2:3]
...
vireo <- subset(final, species == "Vireo_olivaceus")

ggplot(vireo, aes (x = factor(year), y = arr_GAM_mean))+geom_boxplot()+
  scale_y_continuous(trans='log10') 

ggplot(vireo, aes (x = factor(year), y = AnomVArr))+geom_boxplot() 

### what are the latitudinal patterns in arrival speed?

ggplot(final, aes(x = cell_lat, y = vArrMag, level = species,alpha = 0.1)) + 
  geom_smooth(aes(alpha = 0.1, color = "black"), show.legend = FALSE, se = FALSE)

ggplot(final, aes(x = cell_lat, y = vArrMag)) + 
  geom_smooth(aes(alpha = 0.1, color = "black"), show.legend = FALSE, se = FALSE)

### what are the latitudinal patterns in greenup speed?

ggplot(final, aes(x = cell_lat, y = vGrMag)) + 
  geom_smooth(aes(alpha = 0.1, color = "black"), show.legend = FALSE, se = FALSE)


### what are the latitudinal patterns in greenup day, bird arrival day, and lag?

ggplot(final, aes(x = cell_lat, y = gr_mn)) + 
  geom_smooth(aes(alpha = 0.1, color = "black"), show.legend = FALSE, se = FALSE)

ggplot(final, aes(x = cell_lat, y = arr_GAM_mean)) + 
  geom_smooth(aes(alpha = 0.1, color = "black"), show.legend = FALSE, se = FALSE)

ggplot(final, aes(x = cell_lat, y = lag)) + 
  geom_smooth(aes(alpha = 0.1, color = "black"), show.legend = FALSE, se = FALSE)

ggplot(final) + 
  geom_smooth(aes(x = cell_lat, y = gr_mn), show.legend = FALSE, se = FALSE)+ 
  geom_smooth(aes(x = cell_lat, y = arr_GAM_mean, color = "red"), show.legend = FALSE, se = FALSE)

ggplot(final) + 
  geom_smooth(aes(x = cell_lat, y = gr_mn, size = 3, color = "black"), show.legend = FALSE, se = FALSE)+ 
  geom_smooth(aes(x = cell_lat, y = arr_GAM_mean, color = species), show.legend = FALSE, se = FALSE)

#### annual averages

ann_gr_mn <- melt(dcast(final, species ~ year, fun.aggregate = median, na.rm = TRUE,value.var = c("AnomDGr")))
ann_VArr <- melt(dcast(final, species ~ year, fun.aggregate = median, na.rm = TRUE,value.var = c("AnomVArr")))

dat <- data.frame(species = ann_gr_mn$species, year = ann_gr_mn$variable, agm = ann_gr_mn[,3],ava = ann_VArr[,3])

lmr <- lmer(ava ~ agm + (1|species),dat)
summary(lmr)

cor(dat, use = "complete.obs")
plot(ann_gr_mn[55,2:17],ann_VArr[55,2:17])
